...
(function() {
  ...
  function open_dialog(frm, op, idx) {
    ...
    const pending = Math.max(0, req - (flt_zero(op.completed_qty) + flt_zero(op.process_loss_qty)));
    ...
    nts.call({
      ...
      error: function(err) {
        try {
          ...
          if (srv && (String(srv).toLowerCase().indexOf("authorized") !== -1 || String(srv).toLowerCase().indexOf("not authorized") !== -1)) {
            nts.msgprint(String(srv));
          } else if (srv && String(srv).toLowerCase().indexOf("employee") !== -1 && String(srv).toLowerCase().indexOf("not found") !== -1) {
            nts.msgprint(String(srv));
          } else {
