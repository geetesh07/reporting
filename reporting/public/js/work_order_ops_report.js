...
// Final complete UI with rejection reason handling and proper quantity flow
...
(function() {
  if (!document.getElementById("reporting-custom-css")) {
    ...
    s.innerHTML = `
      ...
      .r-rejection-reason{color:#dc3545;font-size:0.85em;font-style:italic}
    `;
  ...
  function build_table(frm, logs_map) {
    ...
    // Find first operation with remaining quantity following ERPNext logic
    ...
    for (let i = 0; i < ops.length; i++) {
      ...
      let pending;
      if (i === 0) {
        // First operation gets full quantity
    ...
    ops.forEach((o, idx) => {
      ...
      if (punches.length) {
        punches.forEach(function(p) {
          ...
          h += `<td class="r-col-rej">${p.rejected_qty || 0}${p.rejection_reason ? '<br><span class="r-rejection-reason">' + escapeHtml(p.rejection_reason) + '</span>' : ''}</td>`;
    ...
    h += `<div class="r-report-note">
      ...
      • Rejection reason required when rejecting quantities<br>
      • System follows ERPNext quantity flow: rejections stay in operation, only completed qty flows to next operation<br>
  ...
  function open_dialog(frm, op, idx) {
    ...
    d.get_field("rej").$input.on("change", function() {
      ...
      if (rej_qty > 0) {
        d.get_field("rejection_reason").df.hidden = 0;
        d.get_field("rejection_reason").df.reqd = 1;
        d.get_field("rejection_reason").refresh();
      } else {
        d.get_field("rejection_reason").df.hidden = 1;
        d.get_field("rejection_reason").df.reqd = 0;
